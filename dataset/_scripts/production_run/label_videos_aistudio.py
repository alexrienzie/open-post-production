#!/usr/bin/env python3
"""Gemini 2.5 Pro production pass — full video catalog.

Inputs:
  - <RUNS_DIR>/production_run/production_manifest.json  (pre-built from catalog)
  - `derivative media/<shoot>/<filename>` proxies, resolved per asset via
    `derivative media/_index/asset_map.json`
  - Long-clip chunks staged in `derivative media/_proxy chunks/`

Differs from the pilot script:
  - reads the production manifest (not the TL pilot 23-clip sample)
  - chunks the 27 long clips into the staging dir for upload
  - --workers N concurrency (default 14) via ThreadPoolExecutor
  - WAL-mode SQLite for concurrent writes
  - separate DB / descriptions / log so it never touches pilot data
"""
...
def assert_env_ready(need_gemini: bool = False, need_proxies: bool = False) -> None:
    if need_gemini and not os.environ.get("GEMINI_API_KEY"):
        sys.exit("GEMINI_API_KEY not set; export it before running gemini-describe")
...
def cmd_gemini_describe(args: argparse.Namespace) -> None:
    """Concurrent describe pass. Each worker owns its own SQLite connection.

    WAL mode lets concurrent writers interleave with low contention. Workers
    catch their own exceptions and persist failure rows; the parent never
    sees them as fatal.
    """
    ...
    LOG.info("gemini-describe: %d chunks pending, %d workers, $%.4f already spent",
             len(rows), workers, total_already_done)

    # Each worker thread builds its own client + connection.
    ...
    def worker(ch_dict):
        ...
        try:
            try:
                cost = _describe_one(con, client, ch_dict, thread_name)
                with _progress_lock:
    ...
    with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="w") as ex:
        ...
        try:
            ...
            for f in futures:
                f.cancel()
            raise
    LOG.info("gemini-describe done: %d done, %d failed", state["done"], state["failed"])


def _describe_one(con: sqlite3.Connection, client, ch: dict, tag: str = "") -> float:
    ...
    video = client.files.upload(file=str(src))
    poll_start = time.time()
    while _state_name(video.state) == "PROCESSING":
        if time.time() - poll_start > 1800:
            raise TimeoutError(f"file {video.name} stuck in PROCESSING > 30 min")
        time.sleep(10)
        video = client.files.get(name=video.name)
    if _state_name(video.state) != "ACTIVE":
        raise RuntimeError(f"upload failed state={_state_name(video.state)}")
    ...
    response = _generate_with_retry(client, video, tag=tag)
    ...
    try:
        ...
        LOG.warning("[%s] delete_file(%s) failed: %s", tag, video.name, e)
    return cost


def _state_name(state) -> str:
    """google-genai returns FileState enum; coerce to plain string."""
    return getattr(state, "name", str(state)).upper()


def _generate_with_retry(client, video, max_attempts: int = 4, tag: str = ""):
    delay = 5.0
    for attempt in range(1, max_attempts + 1):
        try:
            return client.models.generate_content(
                model=GEMINI_MODEL,
                contents=[video, PEGASUS_PROMPT],
                config={
            ...
            msg = str(e).lower()
            transient = any(s in msg for s in ("rate", "quota", "503", "500", "deadline"))
